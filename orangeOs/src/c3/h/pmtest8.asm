...
.2:
	...
	; 在此假设内存是大于 8M 的
	; zhp 计算LinearAddrDemo对应的pagetable，然后把对应的物理页，换成ProcBar所在的物理页
	; 刚好也就是ProcBar的基地址
	; 计算方法，先计算在pde中的偏移x，再计算在pte中的偏移。x*1024*4 + y*4 就等于基于pagetablebase的偏移，加上pagetablebase，就等于flat模式下的绝对地址
	; 覆盖这个位置的pte，就等于重写了对应的物理映射
	; 整理：
	; absolute addr = x*4096 + y*4 + PageTblBase1
