...
PUBLIC void task_tty()
{
	...
	while (1) {
		for (tty = TTY_FIRST; tty < TTY_END; tty++) {
			do {
				tty_dev_read(tty);/*lux: 读取键盘输入，并保存*/
				tty_dev_write(tty);/*lux：回显到屏幕。如果是遇到回车，则通知fs唤起之前一直被阻塞的调用进程*/
			} while (tty->ibuf_cnt);
		}
		...
		case DEV_OPEN:
			...
			break;
		case DEV_READ:
			tty_do_read(ptty, &msg);/*lux：读请求，一般来自fs，会回复fs先阻塞调用方(直到用户输入回车，在tty_dev_write中处理)*/
			break;
		case DEV_WRITE:
			tty_do_write(ptty, &msg);/*lux: 写请求，直接写入显存(显示出来)，并返回*/
			break;
		case HARD_INT:
			/**
...
PRIVATE void tty_dev_read(TTY* tty)
{
	if (is_current_console(tty->console))
		keyboard_read(tty);/*lux: keyboard_read会负责把消息写入tty的缓存*/
}
		...
		if (tty->tty_left_cnt) {
			if (ch >= ' ' && ch <= '~') { /* printable */
				...
				void * p = tty->tty_req_buf +
					   tty->tty_trans_cnt;
				phys_copy(p, (void *)va2la(TASK_TTY, &ch), 1);/*lux: 显示出来*/
				tty->tty_trans_cnt++;
				tty->tty_left_cnt--;
			}
			...
			if (ch == '\n' || tty->tty_left_cnt == 0) {/*lux:回车，准备解释命令*/
				out_char(tty->console, '\n');
				MESSAGE msg;
				msg.type = RESUME_PROC;
				msg.PROC_NR = tty->tty_procnr;
				msg.CNT = tty->tty_trans_cnt;
				send_recv(SEND, tty->tty_caller, &msg);/*lux:发消息给fs,恢复之前被fs阻塞的PROC*/
				tty->tty_left_cnt = 0;
			}
		}
	}
...
PRIVATE void tty_do_read(TTY* tty, MESSAGE* msg)
{
	...
	send_recv(SEND, tty->tty_caller, msg);/*lux: 发消息给调用方，让他先阻塞对应的PROC*/
}
