 ...
 */
static struct proto_ops *pops[NPROTO];//lux 全局协议数组
/*
...
static int get_fd(struct inode *inode)
{
	...
	fd = get_unused_fd();
	if (fd >= 0) {
		struct file *file = get_empty_filp();//lux 获取一个系统打开文件表表项
		...
		file->f_op = &socket_file_ops;//lux 设置操作类
		file->f_mode = 3;
		file->f_flags = O_RDWR;
		file->f_count = 1;
		file->f_inode = inode;
...
struct socket *sock_alloc(void)
{
	...
	sock = &inode->u.socket_i;//socket直接就在inode结构内，这样后面通过inode就可以直接获取到socket
 ...
 */
	...
	if (!(sock = sock_alloc())) //lux 初始化一个socket
	{
 ...
 *	SOCKET module.
 */
 //lux socket协议注册入口（unix proto,inet proto,...)
