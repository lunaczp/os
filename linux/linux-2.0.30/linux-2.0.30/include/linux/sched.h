...
extern inline void select_wait(struct wait_queue ** wait_address, select_table * p)
{
	...
	if (!p || !wait_address)
		return;
	if (p->nr >= __MAX_SELECT_TABLE_ENTRIES)//lux 超过最多可监听的数，返回
		return;
 	entry = p->entry + p->nr;//lux 定位到空闲项
	entry->wait_address = wait_address;//lux 初始化
	entry->wait.task = current;//lux 标记当前进程
	entry->wait.next = NULL;
	add_wait_queue(wait_address,&entry->wait);//lux 插入到资源的wait queue
	p->nr++;
}
