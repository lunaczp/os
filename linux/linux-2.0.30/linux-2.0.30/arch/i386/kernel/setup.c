 ...
 * Setup options
 */
struct drive_info_struct { char dummy[32]; } drive_info;//lux 驱动信息
struct screen_info screen_info;//lux 屏幕信息
#ifdef CONFIG_APM
 ...
 */
#define PARAM	empty_zero_page //lux head.S 会填充相关信息
...
static char command_line[COMMAND_LINE_SIZE] = { 0, };
       char saved_command_line[COMMAND_LINE_SIZE];//lux 内核启动时的完整命令行
//lux 不同cpu架构的初始化函数 main.c会调用
	...
	unsigned long * memory_start_p, unsigned long * memory_end_p)
{
	...
	if (!MOUNT_ROOT_RDONLY)
		root_mountflags &= ~MS_RDONLY;//lux 不是只读，去掉只读标记
	memory_start = (unsigned long) &_end;//lux _end 来自于gcc的定义，是占用内存的末端（前面是.text,.data,.bss,..），即可用内存的开始
	init_task.mm->start_code = TASK_SIZE;//init task (task 0)初始化
	...
	for (;;) {//lux 配置解析，提取mem配置，并处理
		...
		if (c == ' ' && *(const unsigned long *)from == *(const unsigned long *)"mem=") {
			...
			if (!memcmp(from+4, "nopentium", 9)) {//lux 配置了nopentium
				from += 9+4;
				x86_capability &= ~8;
			} else {//lux 其他
