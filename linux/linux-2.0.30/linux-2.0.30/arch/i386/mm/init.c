...
/*	smp_alloc_memory(8192); */
	...
	wp_works_ok = 0;
#endif
	start_mem = PAGE_ALIGN(start_mem);//lux 页对齐
	address = 0;
	pg_dir = swapper_pg_dir;//lux 目录表地址
	while (address < end_mem) {
...
#endif
		/* map the memory at virtual addr 0xC0000000 */ //lux 0xC0000000 = 3G, 3G/4M = 768. 3G映射到pd_dir[768]
		pg_table = (pte_t *) (PAGE_MASK & pgd_val(pg_dir[768]));
		if (!pg_table) {//lux pg_table 还没有初始化，没有对应的物理地址
			pg_table = (pte_t *) start_mem;//lux start_mem开始的4k分配给pg_table
			start_mem += PAGE_SIZE;
		}
		...
		pg_dir++;
		for (tmp = 0 ; tmp < PTRS_PER_PTE ; tmp++,pg_table++) {//lux 初始化1024个pte
			if (address < end_mem)
