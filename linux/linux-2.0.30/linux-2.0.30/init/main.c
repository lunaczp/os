...
static unsigned long memory_start = 0;//lux 可用内存开始地址
static unsigned long memory_end = 0;//lux 可用内存结束地址
 ...
 * enable them
 */
	setup_arch(&command_line, &memory_start, &memory_end);//lux 调用不同cpu架构的初始化函数
...
#endif
	memory_start = kmalloc_init(memory_start,memory_end);//lux 下面是内存的计算及初始化
	sti();
	calibrate_delay();
...
#endif
	mem_init(memory_start,memory_end);//lux 内存初始化
	buffer_init();//lux buffer初始化
	...
	printk(linux_banner);//lux bannder show出来
#ifdef __SMP__
	smp_init();//lux smp 初始化
#endif
	sysctl_init();//lux sysctl 初始化
	/* 
	 *	We count on the initial thread going ok 
	 *	Like idlers init is an unlocked kernel thread, which will
	 *	make syscalls (and thus be locked).
	 */
	kernel_thread(init, NULL, 0);//lux 启动init
/*
