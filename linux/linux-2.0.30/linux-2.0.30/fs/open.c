...
static int do_open(const char * filename,int flags,int mode, int fd)
{
	...
	f = get_empty_filp();//lux 获取系统打开文件表的一个空闲表项
	if (!f)
		return -ENFILE;
	...
	if (flag & O_TRUNC)
		flag |= 2;
	error = open_namei(filename,flag,mode,&inode,NULL);//lux 根据文件名，打开文件（找到inode）
	if (error)
		goto cleanup_file;
	if (f->f_mode & FMODE_WRITE) {
		error = get_write_access(inode);//lux 检查写权限
		if (error)
			goto cleanup_inode;
	}
	...
	if (f->f_op && f->f_op->open) {
		error = f->f_op->open(inode,f);//lux 调用open
		if (error)
			goto cleanup_all;
	}
	f->f_flags &= ~(O_CREAT | O_EXCL | O_NOCTTY | O_TRUNC);

	current->files->fd[fd] = f;//lux 加入当前进程的打开文件数组
	return 0;

cleanup_all:
	if (f->f_mode & FMODE_WRITE)
		put_write_access(inode);//lux 配合get_write_access使用
cleanup_inode:
	iput(inode);//lux 配合open_namei使用
cleanup_file:
	f->f_count--;
	return error;
}
...
int get_unused_fd(void)
{
	...
	if (fd < current->rlim[RLIMIT_NOFILE].rlim_cur) {
		FD_SET(fd, &files->open_fds);//lux 标记占用
		FD_CLR(fd, &files->close_on_exec);//lux 关闭close_on_exec
		return fd;
	}
	return -EMFILE;
}
...
asmlinkage int sys_open(const char * filename,int flags,int mode)
{
	...
	fd = get_unused_fd();//lux 申请一个fd
	if (fd < 0)
		return fd;
	error = getname(filename, &tmp);//lux 格式化文件名
	if (!error) {
		error = do_open(tmp,flags,mode, fd);
		putname(tmp);//lux getname和putname要结合使用，因为getname分配了新页，putname要释放，否则会造成内存泄漏。
		if (!error)
			return fd;//lux 成功，返回fd
	}
	put_unused_fd(fd);//lux 失败，释放资源
	return error;
}
