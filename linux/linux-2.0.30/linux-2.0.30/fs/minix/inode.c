				     ...
				     int silent)
{
	...
	s->s_dev = dev;
	s->s_op = &minix_sops;//lux 设定饿各种inode操作类，下面的iget要用（其实所有的inode操作都要经过s->s_op）
...
static void V1_minix_read_inode(struct inode * inode)
{
	...
	brelse(bh);
	if (S_ISREG(inode->i_mode))//lux 不同文件类型的操作回调不一样
		inode->i_op = &minix_file_inode_operations;//lux 普通文件
	else if (S_ISDIR(inode->i_mode))//lux 目录
		inode->i_op = &minix_dir_inode_operations;
	else if (S_ISLNK(inode->i_mode))//lux 软链
		inode->i_op = &minix_symlink_inode_operations;
	else if (S_ISCHR(inode->i_mode))//lux 字符设备
		inode->i_op = &chrdev_inode_operations;
	else if (S_ISBLK(inode->i_mode))//lux 块设备
		inode->i_op = &blkdev_inode_operations;
	else if (S_ISFIFO(inode->i_mode))//lux fifo队列
		init_fifo(inode);
}
