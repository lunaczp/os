	...
	*result = tmp = (char *) page;
	while (--i) {
		*(tmp++) = c;
		...
		if (!c) {
			*tmp = '\0';
			return 0;//lux 成功，result指向新的文件名，且位于新申请的page
		}
	}
	free_page(page);
	return error;
}
//lux 释放getname申请的page（*name指向该page）
               ...
               struct inode ** res_inode, struct inode * base)
{
	...
	mode |= S_IFREG;
	error = dir_namei(pathname, &namelen, &basename, base, &dir);//lux 获取所在目录
	if (error)
		return error;
	...
	if (flag & O_CREAT) {
		...
		if (!error) {
			...
			error = dir->i_op->create(dir, basename, namelen, mode, res_inode);//lux 创建文件
			up(&dir->i_sem);
			iput(dir);
			return error;
		}
