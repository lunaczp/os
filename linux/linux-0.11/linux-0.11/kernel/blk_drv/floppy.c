/**lux 由于软盘马达在不工作的时候是不转的，所有需要手动开启、关闭马达，而这会造成延迟，所以这部分操作一般是用定时器来完成的。
 * - 读写，读些前，设定定时器来启动马达。
 * - 读写完毕，设定定时器来关闭马达。
 */
/*
...
unsigned char selected = 0;//lux 当前选中的软盘
...
{
	setup_DMA();
	do_floppy = rw_interrupt;//lux 先设置读写回调
	output_byte(command);
...
static void transfer(void)
{
	...
	if (!seek) {//lux 非seek操作，直接读写即可。
		setup_rw_floppy();
		return;
	}//lux 以下是seek操作：
	do_floppy = seek_interrupt;//lux 先设置seek回调
	if (seek_track) {
		output_byte(FD_SEEK);
		output_byte(head<<2 | current_drive);
...
static void recalibrate_floppy(void)
{
	recalibrate = 0;
	current_track = 0;
	do_floppy = recal_interrupt;//lux 重新校准的回调
	output_byte(FD_RECALIBRATE);
	output_byte(head<<2 | current_drive);
	if (reset)
		do_fd_request();
...
static void reset_floppy(void)
{
	...
	cli();
	do_floppy = reset_interrupt;//lux 回调函数：重置软盘
...
void do_fd_request(void)
{
	...
	add_timer(ticks_to_floppy_on(current_drive),&floppy_on_interrupt);//lux 定时启动floppy，并在启动后，执行读写操作。
}

void floppy_init(void)
{
	blk_dev[MAJOR_NR].request_fn = DEVICE_REQUEST;
	set_trap_gate(0x26,&floppy_interrupt);/*Lux 软盘中断*/
