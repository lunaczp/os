...
//lux 编程clock event device设备，指定下一个事件的时间
/**
...
/*lux 为指定的clock event device设置定时模式
 *并设置handler，和next_event
 */
/**
...
void tick_setup_oneshot(struct clock_event_device *newdev,
			...
			ktime_t next_event)
{
	newdev->event_handler = handler;//lux 设定handler
	clockevents_set_mode(newdev, CLOCK_EVT_MODE_ONESHOT);//lux 定时模式
	tick_dev_program_event(newdev, next_event, 1);//lux 设定下一个event
}
//lux 切换到定时模式
/**
...
{
	struct tick_device *td = &__get_cpu_var(tick_cpu_device);//lux 当前cpu的tick设备
	struct clock_event_device *dev = td->evtdev;
