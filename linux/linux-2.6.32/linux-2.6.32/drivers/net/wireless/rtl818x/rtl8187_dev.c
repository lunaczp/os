...
static int rtl8187_tx(struct ieee80211_hw *dev, struct sk_buff *skb)
{
	...
	usb_anchor_urb(urb, &priv->anchored);
	rc = usb_submit_urb(urb, GFP_ATOMIC);//lux send
	if (rc < 0) {
		usb_unanchor_urb(urb);
		kfree_skb(skb);
	}
...
static const struct ieee80211_ops rtl8187_ops = {//lux ieee80211 ops
	.tx			= rtl8187_tx,
	.start			= rtl8187_start,
	.stop			= rtl8187_stop,
				   ...
				   const struct usb_device_id *id)
{
	...
	dev = ieee80211_alloc_hw(sizeof(*priv), &rtl8187_ops);//lux 新建
	if (!dev) {
	...
	err = ieee80211_register_hw(dev);//lux 注册设备
	if (err) {
...
static struct usb_driver rtl8187_driver = {//lux rtl8187 driver
	.name		= KBUILD_MODNAME,
	.id_table	= rtl8187_table,
	.probe		= rtl8187_probe,//lux probe
