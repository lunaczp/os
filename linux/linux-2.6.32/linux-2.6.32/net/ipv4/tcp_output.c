			    ...
			    gfp_t gfp_mask)
{
	...
	skb_push(skb, tcp_header_size);//lux 为头部留位置
	skb_reset_transport_header(skb);//lux 重置头部
	skb_set_owner_w(skb, sk);

	/* Build TCP header and checksum it. */
	th = tcp_hdr(skb);//lux build 头部
	...
	*(((__be16 *)th) + 6)	= htons(((tcp_header_size >> 2) << 12) |
	...
	err = icsk->icsk_af_ops->queue_xmit(skb, 0);//lux 调用network层发包 ops在tcp初始化的时候指定，@see net/ipv4/tcp_ipv4.c:1833
	if (likely(err <= 0))
		return err;
...
/** lux tcp wirte packets to network
			  ...
			  int push_one, gfp_t gfp)
{
	...
	while ((skb = tcp_send_head(sk))) {
		...
		cwnd_quota = tcp_cwnd_test(tp, skb);//lux 还能发多少
		if (!cwnd_quota)
			break;
		...
		if (unlikely(tcp_transmit_skb(sk, skb, 1, gfp)))//lux 把skb发出去
			break;
