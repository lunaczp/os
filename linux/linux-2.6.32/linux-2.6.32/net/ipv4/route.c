...
restart:
	...
	if (!rt_caching(dev_net(rt->u.dst.dev))) {
		...
		if (rt->rt_type == RTN_UNICAST || rt->fl.iif == 0) {
			int err = arp_bind_neighbour(&rt->u.dst);//lux 设置dst的neighbour
			if (err) {
				if (net_ratelimit())
					printk(KERN_WARNING
			...
			*rthp = rth->u.dst.rt_next;
			...
			if (rp)
				*rp = rth;
				...
				skb_dst_set(skb, &rth->u.dst);//lux 记录dst到socket
			return 0;
		}
	 ...
	 */
	if (rt->rt_type == RTN_UNICAST || rt->fl.iif == 0) {
		int err = arp_bind_neighbour(&rt->u.dst);//lux 绑定neighbour
		if (err) {
			spin_unlock_bh(rt_hash_lock_addr(hash));
			    ...
			    unsigned flags)
{
	...
	/* get references to the devices that are to be hold by the routing
	   cache entry */
	rth->u.dst.dev	= dev_out;//lux 设备
	dev_hold(dev_out);
	rth->idev	= in_dev_get(dev_out);
	rth->rt_gateway = fl->fl4_dst;
	rth->rt_spec_dst= fl->fl4_src;

	rth->u.dst.output=ip_output;//lux 发包
	rth->rt_genid = rt_genid(dev_net(dev_out));

	RT_CACHE_STAT_INC(out_slow_tot);

	if (flags & RTCF_LOCAL) {
		rth->u.dst.input = ip_local_deliver;//lux 收包
...
make_route:
	err = ip_mkroute_output(rp, &res, &fl, oldflp, dev_out, flags);//lux 生成路由结果
			  ...
			  const struct flowi *flp)
{
	...
	rcu_read_lock_bh();
	for (rth = rcu_dereference(rt_hash_table[hash].chain); rth;
		rth = rcu_dereference(rth->u.dst.rt_next)) {//查找缓存
...
slow_output:
	return ip_route_output_slow(net, rp, flp);//lux 实时获取rp，并写入缓存
}
			 ...
			 struct sock *sk, int flags)
{
	...
	if ((err = __ip_route_output_key(net, rp, flp)) != 0)//lux 找路由
		return err;
	...
	if (flp->proto) {
		...
		err = __xfrm_lookup(net, (struct dst_entry **)rp, flp, sk,
				    flags ? XFRM_LOOKUP_WAIT : 0);//lux XFRM框架/IPSec
		if (err == -EREMOTE)
